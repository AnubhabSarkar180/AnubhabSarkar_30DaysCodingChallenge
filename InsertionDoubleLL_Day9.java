...
class Result {
 ...
 public static DoublyLinkedListNode sortedInsert(DoublyLinkedListNode llist, int data) {
    ...
    // 2. Handle insertion at the very head
    if (data < llist.data) {
        newNode.next = llist;
        llist.prev = newNode;
        return newNode;
    ...
    // 3. Traverse: stop at the node AFTER WHICH we must insert
    while (current.next != null && current.next.data < data) {
        current = current.next;
    }
    
    // 4. Proper Pointer Handshake
    newNode.next = current.next; // New node points forward to the rest of the list
    newNode.prev = current;      // New node points back to current
    
    if (current.next != null) {
        current.next.prev = newNode; // The 'future' node points back to our new node
    }
    current.next = newNode;          // 'current' node points forward to our new node
